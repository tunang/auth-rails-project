...
import { Trash2, RotateCcw, FolderOpen, Calendar } from "lucide-react";
import { useState, useEffect } from "react";
import { toast } from "sonner";

const DeletedCategoriesModal = () => {
  const [isOpen, setIsOpen] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const dispatch = useAppDispatch();
  const { deletedCategories, isLoadingDeleted, deletedPagination, isLoading, message } = useAppSelector(
    (state) => state.category
  );

  const loadDeletedCategories = (page: number = 1) => {
    const params: PaginationParams = {
      page,
      per_page: 10,
    };

    dispatch(getDeletedCategoriesRequest(params));
  };

  const handleRestoreCategory = (categoryId: number, name: string) => {
    dispatch(restoreCategoryRequest(categoryId));
  };

  useEffect(() => {
    if (isOpen) {
      loadDeletedCategories(1);
      setCurrentPage(1);
    }
  }, [isOpen]);
  ...
  useEffect(() => {
    if (message === "Category restored successfully") {
      toast.success("Đã khôi phục danh mục thành công");
      // Reload deleted categories list after successful restore
      loadDeletedCategories(currentPage);
    }
  }, [message, currentPage]);

  const handlePageChange = (page: number) => {
    setCurrentPage(page);
    loadDeletedCategories(page);
  };
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Danh sách các danh mục đã bị xóa tạm thời. Bạn có thể khôi phục lại các danh mục này.
          </DialogDescription>
        </DialogHeader>
          ...
          {isLoadingDeleted ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-lg font-medium text-gray-500 mb-2">
                Không có danh mục nào đã bị xóa
              </p>
              <p className="text-sm text-gray-400">
                Các danh mục đã xóa sẽ hiển thị ở đây
              </p>
            </div>
          ) : (
