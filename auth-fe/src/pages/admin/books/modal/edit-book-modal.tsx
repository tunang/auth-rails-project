...
import { Pencil, BookOpen, DollarSign, Package, Camera, Users, Tag } from "lucide-react";
...
const EditBookModal = ({ book }: { book: Book }) => {
  ...
  // Load initial data and update form when book changes
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        const [authorsResponse, categoriesResponse] = await Promise.all([
          authorApi.admin.getAuthors({ page: 1, per_page: 50, search: "" }),
          categoryApi.admin.getCategories({ page: 1, per_page: 50, search: "" })
        ]);
        
        // Include current book's authors and categories in the search results
        const allAuthors = [...authorsResponse.data];
        const allCategories = [...categoriesResponse.data];
        
        // Add current book's authors if not already in the list
        book.authors?.forEach(author => {
          if (!allAuthors.find(a => a.id === author.id)) {
            allAuthors.push(author as Author);
          }
        });
        
        // Add current book's categories if not already in the list
        book.categories?.forEach(category => {
          if (!allCategories.find(c => c.id === category.id)) {
            allCategories.push(category as Category);
          }
        });
        
        setSearchedAuthors(allAuthors);
    ...
    // Set preview images
    setCoverImagePreview(book.cover_image_url ? `${import.meta.env.VITE_APP_API_URL_IMAGE}${book.cover_image_url}` : null);
    setSamplePagesPreview(book.sample_page_urls?.map(url => `${import.meta.env.VITE_APP_API_URL_IMAGE}${url}`) || []);
  ...
  const onSubmit = async (data: BookRequest) => {
    ...
    dispatch(updateBookRequest({ id: book.id, data: formData }));
  };
